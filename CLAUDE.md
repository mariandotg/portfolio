...
**Content collections** (`src/content.config.ts`, glob loaders): `notes` (MDX, in `src/content/notes/`), `series` (JSON), `work` (MDX case studies), `testimonials` (JSON). Always filter note listings with `({ data }) => !data.draft`.
