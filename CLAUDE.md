...
This repo hosts the notes (blog) **and** the CV (consolidated from a former separate `cv.marianoguillaume.com` repo). Key routes:
...
- `/notes`, `/notes/archive`, series pages, `/work`, `/about`, `/contact` — as before. The old `/blog/*` URLs 301 to `/notes/*` via `vercel.json`.
...
**Content collections** (`src/content.config.ts`, glob loaders): `notes` (MDX, in `src/content/notes/`), `series` (JSON), `work` (MDX case studies), `testimonials` (JSON). Use `getPublishedNotes(lang)` for note listings so each locale receives only its own content.

### Notes content model — two orthogonal axes

Notes (blog posts) are classified along **two independent dimensions**. They are unrelated; don't conflate them.
...
**Series post count is derived, not stored.** `src/lib/series.ts` computes it by filtering published notes that reference the series. Consequence: a series with **0 published posts is unpublished** — no card in listings and no route (standard or rootLevel). You can commit a series JSON before its first note; it goes live when a non-draft post references it. `getRootLevelSeries()` / `getStandardSeries()` / `getSeriesIndex()` apply that filter. Always go through the helpers, never re-query ad hoc:
- `getSeriesIndex(lang, t)` → the series index: counts posts, drops empty series, sorts by `order`, localizes, builds the `meta` label ("Complete · 3 parts").
- `getSeriesPosts(slug)` → a series' posts sorted by `seriesOrder`.

**Where each page pulls from:**
- `/notes` (`index.astro`) — "Series" section (`getSeriesIndex` → `SeriesCard`) + "Latest Notes" (3 most recent posts → `PostListItem`) + link to archive.
- `/notes/archive` — all posts + collection filter tabs.
- Series landing — banner (6/1 strip) + bilingual header + ordered post list. Rendered by two mirror pages: `notes/series/[slug].astro` (standard) and `[slug].astro` (rootLevel). See "Series routing" above.
...
**Translated notes.** English notes omit `lang` and default to `en`. A Spanish copy uses the filename `<slug>.es.mdx`, sets `lang: "es"`, and sets `translationKey: "<slug>"`. Both versions keep the public slug `<slug>` under `/notes/` and `/es/notes/`. Keep `pubDate`, `collection`, `series`, and `seriesOrder` equal across a translation pair. Use `noteSlug(entry)` when building a note URL.

**Banner effects** (`src/lib/effects/banner/`). Deterministic, dependency-free SVG banners generated from a `bannerSeed` string (same seed → same SVG). The module is a **Strategy registry**: `types.ts` holds the contract (`BannerEffect = (seed, opts?) => string`), each strategy is its own file (`halftone.ts` — the original identicon-style halftone dot grid, currently the only one), and `index.ts` exposes the registry `BANNER_EFFECTS`, the swap constant `ACTIVE_BANNER_EFFECT`, and the facade `bannerSvg`. **Consumers always import `bannerSvg` from `@/lib/effects/banner` — never a strategy module directly.** Changing the site-wide banner look is a one-line edit to `ACTIVE_BANNER_EFFECT`; there is no per-post override and no `bannerEffect` frontmatter field. Strategies must be **pure and isomorphic** (no node-only APIs): `dev/banners.astro` imports the forge inside a client `<script>`, so it gets bundled to the browser. `VIEW_COLS`/`VIEW_ROWS`/`BANNER_BACKDROP` are re-exported from `index.ts` for callers that need to pin the aspect ratio — note cards and post headers must use the same ratio for the banner to line up. Preview/tooling lives at `src/pages/dev/banners.astro` and `src/pages/api/dev/banner-seed.ts`.
...
- Series product decisions (no empty-series pages, no "Featured" badge, no series `collection`, no explicit related-notes list) are recorded in `docs/design/design-system.md` §7 (MDG-135). Check there before reintroducing any of them.
...
- Notes MDX authoring (custom components, `client:*` directives) is documented in `src/components/README.md` and `docs/MDX-COMPONENTS.md`.
- `docs/` also holds product/strategy notes unrelated to the portfolio code; don't treat them as engineering specs.
- `remark-reading-time.mjs` injects reading time into note frontmatter via a remark plugin wired in `astro.config.mjs`.
