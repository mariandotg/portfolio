...
- Series landing — banner + bilingual header + ordered post list (or empty-state when coming soon). Rendered by two mirror pages: `notes/series/[slug].astro` (standard) and `[slug].astro` (rootLevel). See "Series routing" above.
...
- Known nit: `.prose a:hover` in `global.css` is still crimson (shared between notes and work pages); flip to `hsl(var(--primary))` if you want note prose links purple.
