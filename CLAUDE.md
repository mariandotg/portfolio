...
**Banner effects** (`src/lib/effects/banner/`). Deterministic, dependency-free SVG banners generated from a `bannerSeed` string (same seed → same SVG). The module is a **Strategy registry**: `types.ts` holds the contract (`BannerEffect = (seed, opts?) => string`), each strategy is its own file (`halftone.ts` — the original identicon-style halftone dot grid, currently the only one), and `index.ts` exposes the registry `BANNER_EFFECTS`, the swap constant `ACTIVE_BANNER_EFFECT`, and the facade `bannerSvg`. **Consumers always import `bannerSvg` from `@/lib/effects/banner` — never a strategy module directly.** Changing the site-wide banner look is a one-line edit to `ACTIVE_BANNER_EFFECT`; there is no per-post override and no `bannerEffect` frontmatter field. Strategies must be **pure and isomorphic** (no node-only APIs): `dev/banners.astro` imports the forge inside a client `<script>`, so it gets bundled to the browser. `VIEW_COLS`/`VIEW_ROWS`/`BANNER_BACKDROP` are re-exported from `index.ts` for callers that need to pin the aspect ratio — blog cards and post headers must use the same ratio for the banner to line up. Preview/tooling lives at `src/pages/dev/banners.astro` and `src/pages/api/dev/banner-seed.ts`.
