 ...
 *
 * 1. **El prefijo de locale se descarta.** `/blog` y `/es/blog` son la misma página en dos
 *    idiomas, así que comparten campo. Si no se descartara, cambiar de idioma cambiaría el fondo
 *    y parecería un bug.
 * 2. **Es la ruta, no el contenido.** El backdrop hoy vive en 4 rutas fijas (`/cv` y `/blog` por
